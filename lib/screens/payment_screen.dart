...
//  Box<Product> productBox;
  ...
  void initState() {
    ...
    Future.delayed(Duration.zero, () async {
      ...
      /*if(Features.isOffers) {
        Provider.of<BrandItemsList>(context, listen: false).getOffers().then((_) {
  ...
  Future<void> _initial() async {
    debugPrint("member...pay..11"+membershipvx);
 ...
 fetchUserDetail()async{
   ...
   setState(() {
     ...
     debugPrint("member...pay.."+membershipvx);
   ...
   debugPrint("9999....."+delChargeData.itemsDelCharges[0].minimumOrderAmountNoraml.toString());
     if (routeArgs['deliveryType'] == "express") {
       minorderamount = double.parse(_minimumOrderAmountExpress);
       deliverycharge = double.parse(_deliveryChargeExpress);
       note = routeArgs['note'];
     } else {
       if (membershipvx == "1") {
         debugPrint("1111.....");
         _checkmembership = true;
         minorderamount = double.parse(_minimumOrderAmountPrime);
         deliverycharge = double.parse(_deliveryChargePrime);
         note = routeArgs['note'];
       } else {
         debugPrint("0000.....");
         _checkmembership = false;
         minorderamount = double.parse(_minimumOrderAmountNoraml);
         deliverycharge = double.parse(_deliveryChargeNormal);
         note = routeArgs['note'];
         debugPrint("minimumOrderAmountNoraml...."+_minimumOrderAmountNoraml);
       }
     }
     debugPrint("minorderamount..."+minorderamount.toString()+"  "+deliverycharge.toString());
     if (_checkmembership
         ? (CartCalculations.totalMember < minorderamount)
         : (CartCalculations.total < minorderamount)) {
       debugPrint("ghjk....3..");
     ...
     debugPrint("ghjk....1.."+deliveryAmt.toString());
      ...
      setState(() {
        ...
        debugPrint("membership...."+membershipvx);
        if (membershipvx == "1") {
...
debugPrint("hihihi...."+deliveryAmt.toString());
...
/*      if(PrefUtils.prefs.containsKey("orderId")) {
  ...
  Widget _myRadioButton({String title, int value, Function onChanged}) {
    ...
    return Theme(
      data: ThemeData(
        unselectedWidgetColor: ColorCodes.discountoff,
      ),
      child: Radio(
  ...
  _addToCart() async {
    ...
    await Provider.of<CartItems>(context, listen: false).addToCart(
     ...
     // productBox.add(products);
      /* Orderfood();*/
      if(_slots){
        debugPrint("not only membership...");
        if(_isPickup){
          debugPrint(" Pickup...");
          Orderfood();
        }else {
          debugPrint("not Pickup...");
          if(Features.isSplit){
            debugPrint("Split...");
            OrderfoodSplit();
          }else{
            debugPrint(" not split...");
            Orderfood();
          }
        }
      }else{
 ...
 /* removeToCart(itemCount) async {
                      ...
                      children: <Widget>[
                        ...
                        GestureDetector(
                          onTap: () {
                            ...
                            // Navigator.of(context).pop();
                            // Navigator.of(context).pop();
                            debugPrint("abc..def..."+routeArgs['minimumOrderAmountNoraml']+"  "+routeArgs['deliveryCharge']);
                            PrefUtils.prefs.setString("addressbook","payment");
                            Navigator.of(context)
  ...
  //  try {
      ...
      resBody["addressId"] = PrefUtils.prefs.getString('addressId') /*routeArgs['addressId']*/;
      resBody["orderType"] = _isPickup ? "pickup" : (routeArgs['deliveryType'] == "Default") ? "Delivery": "express" ;
      resBody["paymentType"] = PrefUtils.prefs.getString('payment_type');
      resBody["promocode"] = (routeArgs['fromScreen'].toString() == "promocodeScreen")?routeArgs['promocode']:"";
      resBody["fix_time"] = /*_isPickup ? PrefUtils.prefs.getString('fixtime')
          : (routeArgs['deliveryType'] == "OptionTwo") ? deliveryDurationExpress+" - express" : PrefUtils.prefs.getString('fixtime')*/"6:00 AM - 9:00 PM";
      resBody["fixdate"] = /*_isPickup ? PrefUtils.prefs.getString('fixdate') : (routeArgs['deliveryType'] == "OptionTwo") ?*/ finalDate /*: PrefUtils.prefs.getString('fixdate')*/;
      resBody["promo"] = (routeArgs['fromScreen'].toString() == "promocodeScreen")?routeArgs['promocode']:"";
//      resBody["membership"] = PrefUtils.prefs.getString('fixtime');
      ...
      resBody["note"] = note;
      resBody["branch"] = PrefUtils.prefs.getString("branch");
      ...
      resBody["giftWrapamount"] = PrefUtils.prefs.getString("gift") == "1" ? IConstants.giftWrapamount : "0";
      resBody["giftWrap"] =  PrefUtils.prefs.getString("gift");
      // resBody["giftNote"] =



      for (int i = 0; i < productBox.length; i++) {
        debugPrint("product....."+productBox[i].itemId.toString());
  ...
  Future<void> OrderfoodSplit() async {
    ...
    var url = Api.newOrderByCartSplit;
    String channel = "";
    var membership = "";
    var index = "";
    String type = "1";
...
//      resBody["membership"] = PrefUtils.prefs.getString('fixtime');
      ...
      resBody["expressDeliveryCharge"] = routeArgs['deliveryChargeExpress'];

      if(membershipvx == "1") {
        resBody["normalDeliveryCharge"] = routeArgs['deliveryChargePrime'];
      }else{
        resBody["normalDeliveryCharge"] =  routeArgs['deliveryChargeNormal'];
      }
        ...
        if(double.parse(productBox[i].varStock ) > 0 && productBox[i].status.toString() == "0") {
          if (index != "") {
            if (int.parse(index) == i) {}
              ...
              if (routeArgs['deliveryType'] == "Default") {
                resBody["items[" + i.toString() + "][orderType]"] =
                _isPickup ? "pickup" : "Delivery";
              }
              else if (routeArgs['deliveryType'] == "OptionTwo") {
      ...
      if (responseJson['status'].toString() == "true") {
        ...
        if (PrefUtils.prefs.getString('payment_type') == "paytm") {
          ...
          final routeArgs = ModalRoute.of(context).settings.arguments as Map<String, String>;
          Navigator.of(context).pop();
          payment.startPaytmTransaction(context,  _isWeb,orderId: orderdecode['ref_id'].toString(), username: PrefUtils.prefs.getString('userID'),amount: orderAmount, routeArgs: routeArgs,prev:"PaymentScreen");
  ...
  //       // Navigator.of(context).pop();
  //     });
  //     return Fluttertoast.showToast(msg: S.of(context).please_enter_valid_promo,//"Please enter a valid Promocode!!!",
  //       fontSize: MediaQuery.of(context).textScaleFactor *13,);
  //   } else {
  //     var item = [];
  //     for (int i = 0; i < productBox.length; i++) {
  //       if (_checkmembership == true) {
  //         if (double.parse(productBox[i].price) <= 0 ||
  //             productBox[i].price.toString() == "" ||
  //             productBox[i].price ==
  //                 productBox[i].varMrp) {
  ...
  //         debugPrint("promocode..."+responseJson['prmocodeType'].toString());
  ...
  //           });
  //           debugPrint("asdfghkfdfdg"+ responseJson['prmocodeType'].toString());
  //          // _dialogForPromo()  ;
  //
  //         }
  //         else {
  ...
  //         });
  //         //Navigator.of(context).pop();
  //         return Fluttertoast.showToast(msg: responseJson['msg'].toString(), fontSize: MediaQuery.of(context).textScaleFactor *13,);
  //       }
  //
  //     } catch (error) {
  //       throw error;
  ...
  _dialogforPromo (String promocode, String amount){
    showDialog(
                ...
                Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 10,),
                      Text("Keep using "+ promocode +" and save more with each order", textAlign: TextAlign.center, style: TextStyle(color: ColorCodes.blackColor,
                          fontSize: 17), ),
                      SizedBox(height:17),
                      Text(
                        S.of(navigatorKey.currentContext).yay,
                        textAlign: TextAlign.center,
                        style: TextStyle(fontWeight: FontWeight.bold,fontSize: 20,color: ColorCodes.discountoff),),
                    ],
                  ),
                ),
              ],
  ...
  Future<void> LogicPromo() async{
    final routeArgs = ModalRoute.of(context).settings.arguments as Map<String, dynamic>;
    // final response = json.encode(routeArgs['responsejson']);
    // final responseJson = json.decode(response);

     debugPrint("ststus..."+routeArgs['prmocodeType']);
    if (membershipvx == "1") {
      debugPrint("totalMember...."+ CartCalculations.totalMember.toString());
      cartTotal = CartCalculations.totalMember;
    } else {
    ...
    if (routeArgs['status'] == "done") {
      _dialogforPromo(routeArgs['promocode'],routeArgs['amount']);
    // debugPrint("ststus..."+responseJson.toString());
      if ((routeArgs['prmocodeType'] == "cashback") || (routeArgs['prmocodeType'] == "Cashback")){
        setState(() {
          _checkpromo = false;
          _displaypromo = true;
          _savedamount = "0.0";
          ...
          promocashbackmsg = routeArgs['msg'];
          promomessage = routeArgs['prmocodeType'];
        });}
      else {
        setState(() {
          _checkpromo = false;
          _displaypromo = true;
          _savedamount = routeArgs['amount'];
          /* if (_isPickup)
                _promoamount = (cartTotal -
                    double.parse(responseJson['amount'].toString()))
                    .toString();
      ...
      //Apply loaylty after applied promocde
      if (_isLoayalty &&
          routeArgs['prmocodeType'] != "cashback") {
        await Provider.of<BrandItemsList>(context,listen: false)
            .checkLoyalty(_promoamount)
            .then((_) {
          setState(() {
    ...
    else if (routeArgs['status'] == "error") {
      setState(() {
        _checkpromo = false;
        _displaypromo = false;
        _savedamount = "";
      ...
      //Navigator.of(context).pop();
      return Fluttertoast.showToast(msg: routeArgs['msg'], fontSize: MediaQuery.of(context).textScaleFactor *13,);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    for(int i=0;i<cartItemsData.items.length;i++)
      ...
      if (deliveryAmt <= 0) {
        ...
        deliverychargetext = IConstants.numberFormat == "1"?"+ " + IConstants.currencyFormat + " " + deliveryAmt.toStringAsFixed(0):
        "+ " + IConstants.currencyFormat + " " + deliveryAmt.toStringAsFixed(IConstants.decimaldigit);
      }

    debugPrint("deliveryAmt..."+deliveryAmt.toString());
  /*  if (deliveryAmt <= 0) {
      deliverychargetext = "FREE";
    }
    else {
...
*/
    ...
    _buildBottomNavigationBar() {
      final routeArgs = ModalRoute.of(context).settings.arguments as Map<String, dynamic>;
      debugPrint("_promoamount.....1"+_promoamount.toString() +"  "+cartTotal.toString()+" "+_displaypromo.toString());
      return  _isLoading
          ? Shimmer.fromColors(
      ...
      BottomNaviagation(
        ...
        title: S.current.place_order, //'Place Order',
        total: !_displaypromo ? _isPickup  ? _isSwitch && !_isLoyaltyToast && _isLoayalty &&
            (double.parse(loyaltyPoints) > 0)  ?
      ...
      return AppBar(
        ...
        leading: IconButton(
            ...
            onPressed: () {
              ...
              }else if(routeArgs['fromScreen'] == "addaddress"){
                debugPrint("back....1");
                Navigator.of(context).pushReplacementNamed(
                    CartScreen.routeName,arguments: {
                  "after_login": ""
    ...
    Widget _bodyMobile() {
     ...
     Widget paymentMode() {
                                  ...
                                  if (_ischeckboxshow && _ischeckbox)
                                    Text(
                                      IConstants.currencyFormat +
                                          ...
                                          walletAmount.toStringAsFixed(IConstants.numberFormat == "1"?0:IConstants.decimaldigit),
                                      style: TextStyle(
                                          color: (walletbalance == "0")? ColorCodes.grey : ColorCodes.discountoff,
                                          fontSize: 18.0,
                                          fontWeight: FontWeight.bold),
                                    ),
                                  SizedBox(width: 5.0),
                      ...
                      child: Row(
                          ...
                          children: <Widget>[
                            ...
                            Row(
                              children: [
                                Icon(Icons.check_box_outline_blank,
                                    size: 25.0, color: (walletbalance == "0")? ColorCodes.grey : ColorCodes.discountoff)
                              ],
                            ),
                           // SizedBox(width: 20,)
                          ],
                  ...
                  child: Container(
                    ...
                    child: Row(
                      ...
                      children: <Widget>[
                        ...
                        (routeArgs['fromScreen'].toString() == "promocodeScreen")?
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              Text(
                                routeArgs['promocode'],
                                style: TextStyle(fontSize: 16, color: ColorCodes.blackColor, fontWeight: FontWeight.bold),
                        ...
                        :Container(
                          ...
                          child: Text(
                            "Apply Coupon",
                            style: TextStyle(fontSize: 19, color: ColorCodes.blackColor, fontWeight: FontWeight.bold),
                          ),
                        ...
                        MouseRegion(
                          cursor: SystemMouseCursors.click,
                          child:(routeArgs['fromScreen'].toString() == "promocodeScreen")?
                          ...
                          GestureDetector(
                            ...
                            child: Container(
                             ...
                             color: ColorCodes.discountoff,fontWeight: FontWeight.bold
                                /*Icon(Icons.keyboard_arrow_right_sharp, size: 35, color: ColorCodes.greenColor,),*/),
                            ))),
                          )
                          ...
                          :GestureDetector(
                            onTap: () {
                              final routeArgs = ModalRoute.of(context).settings.arguments as Map<String, dynamic>;
                              Navigator.of(context).pushReplacementNamed(PromocodeScreen.routeName,arguments: {
                                "deliveryAmt": deliveryAmt.toString(),
                                'minimumOrderAmountNoraml': routeArgs['minimumOrderAmountNoraml'],
                                'deliveryChargeNormal': routeArgs['deliveryChargeNormal'],
                                'minimumOrderAmountPrime': routeArgs['minimumOrderAmountPrime'],
                                'deliveryChargePrime': routeArgs['deliveryChargePrime'],
                              ...
                              debugPrint("mini...."+routeArgs['minimumOrderAmountNoraml']);
                            },
                            child: Container(
                              padding: EdgeInsets.only(left: 5,right: 5),
                              width: (MediaQuery.of(context).size.width / 5.1),
      ...
      Widget promocodeMethod() {
       print("savings.....total.."+(CartCalculations.totalmrp - (amountPayable-deliveryAmt)).toString()+"  "+CartCalculations.totalmrp.toString()+"  "+amountPayable.toString()+ " "+deliverycharge.toString()+"  "+deliveryAmt.toString());
        if( deliverychargetext=="FREE"){
      ...
      Widget paymentDetails() {
       debugPrint("membership...price.."+CartCalculations.totalMembersPrice.toString()+ " " +CartCalculations.totalprice.toString()+"  "+
           CartCalculations.totalmrp.toString() +"  "+ cartTotal.toString());
        return Column(
          children: [
            Container(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text("Price Details" + " " + "(" + CartCalculations.itemCount.toString() + " " + "Items" + ")",
                          ...
                          child: Column(
                            ...
                            //   ),
                              ...
                              if (PrefUtils.prefs.getString("gift") == "1")
                                Text("Gift Wrap",  style: TextStyle(
                              ...
                              if (_displaypromo &&
                                  promomessage.toString().toLowerCase() !=
                                      'cashback' ||  (routeArgs['fromScreen'].toString() == "promocodeScreen"))
                                new RichText(
                                  text: new TextSpan(
                                    style: TextStyle(
                                        fontSize: 14.0,
                              ...
                              Text(
                                S.of(context).total_amount,//"Total Amount",
                                style: TextStyle(
                                  fontSize: 17.0,
                                  color: ColorCodes.blackColor,
                        ...
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              if ((CartCalculations.totalprice) > 0)
                                Text(
                                  PrefUtils.prefs.getString("gift") == "1" ? "- " +
                                       (CartCalculations.totalmrp - cartTotal + int.parse(IConstants.giftWrapamount))
                                          .toStringAsFixed(IConstants.numberFormat == "1"?0:IConstants.decimaldigit) : "- " + (CartCalculations.totalmrp - cartTotal).toStringAsFixed(IConstants.numberFormat == "1"?0:IConstants.decimaldigit),
                                  style: TextStyle(
                                      fontSize: 13.0,
                                      color: ColorCodes.discountoff,
                                      fontWeight: FontWeight.bold),
                              ...
                              if (PrefUtils.prefs.getString("gift") == "1")
                                Text("+" + " " + IConstants.giftWrapamount,  style: TextStyle(
                                    fontSize: 13.0, color: ColorCodes.greyColor,
                                    fontWeight: FontWeight.bold)),
                              if (PrefUtils.prefs.getString("gift") == "1")
                              SizedBox(
                          ...
                          Row(
                            children: [
                              ...
                              GestureDetector(
                                ...
                                child: Text(
                                  "Change",//"Select delivery address",
                                  style: TextStyle(
                                      fontWeight: FontWeight.bold,
                                      fontSize: 14.0,
                                      color: ColorCodes.discountoff),
                              ...
                              Container(
                                child:  Text(addressdata.billingAddress[0].addressType,
                                  style:new TextStyle(fontSize:11,
                                    fontWeight: FontWeight.w500,
                                    color: ColorCodes.greyColor, background: Paint()..color = ColorCodes.searchwebbackground
                                      ..strokeWidth = 12
                          ...
                          Container(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                //   children: [
                                //     Expanded(
                                //       child:  new RichText(textAlign: TextAlign.start,
                                //         text: new TextSpan(
                                //
                                //           style: new TextStyle(
                                //               fontSize:13,
                                ...
                                //           children: <TextSpan>[
                                //             new TextSpan(
                                //                 text: addressdata.billingAddress[0].houseno + ", " + addressdata.billingAddress[0].area + "," + addressdata.billingAddress[0].landmark + " " + addressdata.billingAddress[0].city + ", " + addressdata.billingAddress[0].state +  "\n",
                                //                 style:new TextStyle(fontSize: 13, color: ColorCodes.darkGrey, fontWeight: FontWeight.normal)
                                //
                                //             ),
                                //
                                //             new TextSpan(
                                //                 text: "Pin Code: " + addressdata.billingAddress[0].pincode +  "\n",
                                //                 style:new TextStyle(fontSize: 13, color: ColorCodes.darkGrey, fontWeight: FontWeight.normal)
                                //
                                //             ),
                                //             new TextSpan(
                                //                 text: "Mobile: " + addressdata.billingAddress[0].mobile +  "\n",
                                //                 style:new TextStyle(fontSize: 13, color: ColorCodes.darkGrey, fontWeight: FontWeight.normal)
                                //             ),
                                //
                                //           ],

...
import 'package:getit/screens/singleproductimage_screen.dart';
...
enum Routename{
  ...
  SubscriptionBoxScreen,SubscribeDescription,EditSubscription,SingleProductImageScreen
...
class PageControler extends Navigations  {
  ...
  return HomeScreen(key: state.pageKey);},
     ...
     // GoRoute(name: nUri(Routename.NotAvailability).name,path: nUri(Routename.NotAvailability).path+"/:val/:currentbranch/:item", builder: (context, state) =>NotavailabilityProduct(state.params),),
      ...
      GoRoute(name: nUri(Routename.SingleProductImageScreen).name,path: nUri(Routename.SingleProductImageScreen).path, builder: (context, state) =>SingleProductImageScreen(state.queryParams),),
    ])
  ];
}
class CUri{
...
abstract class Navigations {
  ...
  CUri  Function(Routename name) get nUri=>(Routename name){
    switch(name){
      ...
      case Routename.SingleProductImageScreen:
        return CUri('SingleProductImageScreen');
    }
  };
